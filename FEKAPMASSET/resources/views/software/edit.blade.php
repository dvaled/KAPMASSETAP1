...
<div id="createMaster" class="inset-0 bg-white flex justify-center items-center p-4">
    <div class="bg-white p-6 rounded-md w-96">
        ...
        <form  method="POST" enctype="multipart/form-data" action="{{ route('detailAsset.software.update', ['assetcode' => $assetcode]) }}">
            @csrf
            @method('PUT')
            {{-- assetcode --}}
            <div class="mb-4">
                <label for="idasset" class="block text-sm font-semibold">Software ID</label>
                <input id="idasset" name="idasset" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" readonly value="{{ $assetid }}"></input>
            </div>
            <div class="mb-4">
